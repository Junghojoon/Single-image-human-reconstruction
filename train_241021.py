...
import sys
from scene.__init__241021 import Scene, GaussianModel
...
def tensor_to_image(tensor, file_name):
    ...
    if len(tensor.shape) == 3 and tensor.shape[0] in [1, 3]: 
        tensor = tensor.permute(1, 2, 0)  # (채널, H, W) -> (H, W, 채널)

    tensor = tensor.numpy()
    tensor = (tensor - tensor.min()) / (tensor.max() - tensor.min())  # 정규화
    tensor = (tensor * 255).astype(np.uint8)  # [0, 255]로 스케일링 후 uint8로 변환
...
def training(dataset: ModelParams, opt: OptimizationParams, pipe: PipelineParams, is_pbr=False):
    ...
    if args.type in ['bind', 'bind_mesh_adapt']:
        ...
        gaussians.adaptive_bind(
                use_offset=False,  # Free points or triangle points + free offset
                fix_gaus=False,  # Free points
                anchor_field=True, 
    ...
    """ Prepare render function and background color """
    ...
    # Training 루프에서 viewpoint_cam의 추가 정보를 출력
    for iteration in progress_bar:
        gaussians.update_learning_rate(iteration)

        # Every 1000 iterations increase SH levels up to a maximum degree
        if iteration % 1000 == 0:
        ...
        # Instead of using randint, we pick the camera sequentially
        viewpoint_cam = viewpoint_stack.pop(0)      
        
        print(viewpoint_cam)
        # print(viewpoint_cam.shape)
        # input()  
            
        # GT Image Path와 함께 Camera 정보 및 input image path 출력
        print(f"def_training_Iteration {iteration}")
        print(f"GT Image Path: {viewpoint_cam.image_path}")  # GT 이미지 경로
        # print(f"Camera Info - R: {viewpoint_cam.R}, T: {viewpoint_cam.T}, K: {viewpoint_cam.K}")  # 카메라 R, T, K 정보
        print(f"Camera Position: ({viewpoint_cam.cx}, {viewpoint_cam.cy}), Focal Lengths: ({viewpoint_cam.fx}, {viewpoint_cam.fy})")
        print(f"Input Image Path: {viewpoint_cam.image_path}")  # input 이미지 경로 (같은 경로 사용)
        # 추가적으로 필요하면 viewpoint_cam에서 더 많은 정보 추출 가능
        # input()  # 확인을 위해 일시 정지할 수 있음
            ...
            if iteration % args.checkpoint_interval == 0 or iteration == args.iterations:
                ...
                for com_name, component in pbr_kwargs.items():
                    try:
                        ...
                        print(f"\n[ITER {iteration}] Saving {com_name} Checkpoint")
                    except:
                        pass
    ...
    if args.type in ["render"]:
        ...
        mesh = gaussians.extract_mesh(mesh_path, density_thresh=0.0001, resolution=256, num_blocks=16, relax_ratio=0.5)
        ...
        for config in validation_configs:
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(
                    ...
                    print(f"def_training_report_fps_Iteration {iteration}, GT Image Path: {viewpoint.image_path}")  # GT 이미지 경로 출력
                    # input()
    ...
    if iteration % args.test_interval == 0:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(
                        tqdm(config['cameras'], desc="Evaluating " + config['name'], leave=False)):
                    print(f"def_training_report_Iteration {iteration}, GT Image Path: {viewpoint.image_path}")  # GT 이미지 경로 출력
                    # input()
                    render_pkg = renderFunc(viewpoint, scene.gaussians, pipe, bg_color,
                                            scaling_modifier, override_color, opt, is_training,
                                            **kwargs)
...
def eval_render(scene, gaussians, render_fn, pipe, background, opt, pbr_kwargs):
    ...
    # 디렉터리 생성 코드 추가
    render_save_dir = os.path.join(args.model_path, 'eval', 'render', 'images', '00')
    create_folder_if_not_exists(render_save_dir)  # 경로가 없으면 생성합니다.
    ...
    with torch.no_grad():
        for idx in progress_bar:
            ...
            # save_image 호출 전에 경로가 유효한지 확인
            save_image(image, os.path.join(render_save_dir, f"{viewpoint.image_name}.png"))
...
if __name__ == "__main__":
    ...
    (f"Current model path: {args.model_path}")
